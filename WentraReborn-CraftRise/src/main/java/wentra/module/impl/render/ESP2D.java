...
public class ESP2D extends Module {
    ...
    public ESP2D() {
        super("ESP2D", ModuleCategory.RENDER, 0);
        this.toggle();
    }
    ...
    private void renderBoxNameTags() {
        if (Mapper.EntityClass == null || Mapper.EntityPlayerSP == null) {
            System.err.println("Required mappings are null, skipping renderBoxNameTags");
            return;
        }
        ...
        try {
            ...
            for (Map.Entry<Object, double[]> entry : entityConvertedPointsMap.entrySet()) {
                ...
                if (renderPositions == null || renderPositions.length < 25) continue;

                double[] renderPositionsBottom = {renderPositions[4], renderPositions[5], renderPositions[6]};
                double[] renderPositionsX = {renderPositions[7], renderPositions[8], renderPositions[9]};
                double[] renderPositionsX1 = {renderPositions[10], renderPositions[11], renderPositions[12]};
                double[] renderPositionsZ = {renderPositions[13], renderPositions[14], renderPositions[15]};
                double[] renderPositionsZ1 = {renderPositions[16], renderPositions[17], renderPositions[18]};
                double[] renderPositionsTop1 = {renderPositions[19], renderPositions[20], renderPositions[21]};
                double[] renderPositionsTop2 = {renderPositions[22], renderPositions[23], renderPositions[24]};

                boolean shouldRender = renderPositions[3] > 0.0 && renderPositions[3] <= 1.0 &&
                        renderPositionsBottom[2] > 0.0 && renderPositionsBottom[2] <= 1.0 &&
                        renderPositionsX[2] > 0.0 && renderPositionsX[2] <= 1.0 &&
                        renderPositionsX1[2] > 0.0 && renderPositionsX1[2] <= 1.0 &&
                        renderPositionsZ[2] > 0.0 && renderPositionsZ[2] <= 1.0 &&
                        renderPositionsZ1[2] > 0.0 && renderPositionsZ1[2] <= 1.0 &&
                        renderPositionsTop1[2] > 0.0 && renderPositionsTop1[2] <= 1.0 &&
                ...
                try {
                    if (Mapper.EntityClass.isInstance(entity) && !Mapper.EntityPlayerSP.isInstance(entity)) {
                        ...
                        double[] xValues = {renderPositions[0], renderPositionsBottom[0], renderPositionsX[0],
                                renderPositionsX1[0], renderPositionsZ[0], renderPositionsZ1[0], renderPositionsTop1[0],
                                renderPositionsTop2[0]};
                        double[] yValues = {renderPositions[1], renderPositionsBottom[1], renderPositionsX[1],
                                renderPositionsX1[1], renderPositionsZ[1], renderPositionsZ1[1], renderPositionsTop1[1],
                                renderPositionsTop2[1]};

                        double x = Arrays.stream(xValues).min().orElse(renderPositions[0]);
                        double y = Arrays.stream(yValues).min().orElse(renderPositions[1]);
                        double endx = Arrays.stream(xValues).max().orElse(renderPositionsBottom[0]);
                        double endy = Arrays.stream(yValues).max().orElse(renderPositionsBottom[1]);

                        Color boxColor = Color.WHITE;
                        rectangleBorderedESP(x + 0.5, y + 0.5, endx - 0.5, endy - 0.5, 1.0, getColor(0, 0, 0, 0), boxColor.getRGB());

                        float health = Entity.getEntityHealth(entity);
                        float[] fractions = {0.0f, 0.5f, 1.0f};
                        Color[] colors = {Color.RED, Color.YELLOW, Color.GREEN};
                        float progress = health / 20.0f;
                        ...
                        RenderUtil.drawRectESP((int) (x - 5.5), (int) (endy - 1.0), (int) (x - 3.5), (int) healthLocation, customColor.getRGB());

                        double fontScale = 1.10;
                        float middle = (float) (x + (endx - x) / 2);
                        String name = Entity.getName(entity);
                        String cleanName = Entity.cleanDisplayName(name);
                        String text = "§f" + cleanName;

                        float textWidth = (int) Mapper.getStringWidth.invoke(Render2DHelper.fontRenderer, text);

                        middle -= (textWidth * fontScale) / 2f;
                        double fontHeight = 9 * fontScale;
                        ...
                        try {
                            ...
                            FontRenderer.drawText(text, (int) middle, (int) (y - (fontHeight + 4)), Color.WHITE.getRGB());
                        } finally {
                            GL11.glPopMatrix();
                        }
                    }
